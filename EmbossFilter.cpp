...
void EmbossFilter::UnUse()
{
	assert(this->is_used == true);
	this->GLReturnedError("EmbossFilter::UnUse() - entering");
	Shader::UnUse();
}

void EmbossFilter::CustomSetup()
{
	Shader::Use();
	uniforms.image_size = glGetUniformLocation(this->program_id, "image_size");
	uniforms.texture = glGetUniformLocation(this->program_id, "texture");
	Shader::UnUse();
}

void EmbossFilter::EF(EmbossFilter & ef, FrameBufferObject & fbo, bool copy_back, int image_size, int number_of_samples)
{
	...
	ef.Use();
	glUniform1i(ef.uniforms.image_size, ef.image_size = image_size);
	glUniform1i(ef.uniforms.texture, 0);
	...
	if (copy_back)
	{
		...
		glBlitFramebuffer(0, 0, ef.image_size, ef.image_size, 0, 0, ef.image_size, ef.image_size, GL_COLOR_BUFFER_BIT, GL_LINEAR);
	}
